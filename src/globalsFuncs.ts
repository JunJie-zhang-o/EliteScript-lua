...
export function addGloabalsFuncs() {
    ...
    let globalsValue = vscode.workspace.getConfiguration().get("Lua.diagnostics.globals") as string[]
    
    var needGlobalsFuncs = globalsValue.slice();
    var isConfig = false
    function findNeedConfigFuncs(items:string, index:number) {
        if (globalsValue.includes(items) === false) {
            isConfig = true
            needGlobalsFuncs.push(items)
        }
    }
    function removeECScript(name:string, index:number){
        if (eliteFuncs.includes(name)){
            isConfig = true
            needGlobalsFuncs = needGlobalsFuncs.filter(item => item !== name)
        }
    }
    // eliteFuncs.forEach(findNeedConfigFuncs)
    globalsValue.forEach(removeECScript)
    if (isConfig) {
